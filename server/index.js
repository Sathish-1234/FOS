...
app.post("/insertproduct", upload.single("image"), (req, res) => {
	...
	db.query(insertMenuItemQuery, menuItemValues, (err, result) => {
		...
		try {
			...
			console.log(parsedBases, parsedToppings);
			//push toppings data
...
app.post("/orderdetails", (req, res) => {
	...
	const values = cart.map(({ menuid, menuitemid, count, menuitem, subtotal, toppingsid, baseid}) => [
		customerID,
		orderNumber,
		addressID,
		menuid,
	...
	const sql =
		"INSERT INTO ORDERDETAILS (customerID, orderNumber, addressId, menuid, menuitemid, quantity, menuitems, price,pickuptime, toppingsId, baseId) VALUES ?";
	...
	db.query(sql, [values], (error, results) => {
		if (error) {
			...
			const invoiceSql =
				"INSERT INTO INVOICEDETAILS (invoiceNumber,orderNumber, customerID, addressId,totalBill,orderstatus,purchasemode,transcationmode) VALUES ?";
			const invoiceValues = [
				[
					invoiceNumber,
					orderNumber,
...
app.get("/gettodayinvoice", (req, res) => {
	const date = req.query.selectedDate; // Access the date from the query parameters
	const sql = `SELECT * FROM INVOICEDETAILS WHERE DATE(invoiceDate) = ?`;
...
app.get("/getacceptedindividualorders/:id", (req, res) => {
	...
	db.query(sql, [dpId, "accepted"], (err, data) => {
		if (err) return res.json(err);
		return res.json(data);
	});
});
...
app.put("/updateIssuesstatus/:id", (req, res) => {
	...
	console.log(invoiceNo, selectedIssuesOption);
	const status = "issues";
	const sql =
